...
class magdroneControlNode():
    ...
    def __init__(self):
        ...
        self.struct_z = 2.00
        # Mission 1.
        self.desired_positions_m1 = [1.5]                             #Just give one height from ground initially
        # # Mission 2                                                                                       #Mission2 not required for now
        # self.desired_positions_m2 = [0.4, 0.15, 0.4, 0.7, 1.0, 1.4]

        # Constant transformations                                                      #This is the Constant HD_C matrix[R,T]
        self.q_CwD = [0, -0.7071, 0.7071, 0]
        self.q_CwD_i = [0, 0.7071, -0.7071, 0]
        self.t_CwD = [1, -0.025, -0.085, 0.10]

        # Set up Subscribers
        self.stag_sub = rp.Subscriber(
            "/stag_ros/markers", PoseStamped, self.stag_callback, queue_size=1)
    ...
    def send_attitude_target(self, roll_angle=0.0, pitch_angle=0.0,
        ...
        msg = self.vehicle.message_factory.set_attitude_target_encode(
            ...
            1,  # Target system
            1,  # Target component
            0b00000000 if use_yaw_rate else 0b00000100,          #type mask
    ...
    def set_attitude(self, roll_angle=0.0, pitch_angle=0.0,
        ...
        while time.time() - start < duration:                                                                           #Why to use duration?]
            self.send_attitude_target(roll_angle, pitch_angle,
                                      yaw_angle, yaw_rate, use_yaw_rate,
                                      thrust)
            time.sleep(duration)
...
#                 data.pose.position.z]
    ...
    def stag_callback(self, data):
        q_BwC = [data.StagArray.pose.orientation.w,
                 data.StagArray.pose.orientation.x,
                 data.StagArray.pose.orientation.y,
                 data.StagArray.pose.orientation.z]

        t_BwC = [0,
                 data.StagArray.pose.position.x,
                 data.StagArray.pose.position.y,
                 data.StagArray.pose.position.z]
...
#**********************************************************************************
        ...
        self.y_error = drone_error[1]                                                                           #if vision based why opti?
        self.z_error = dz
    ...
    def send_commands(self):
        ...
        while not rp.is_shutdown():
            ...
            if self.on_mission:
                ...
                if (self.mission_id == 1) & (self.state_id == len(self.desired_positions_m1) - 2):

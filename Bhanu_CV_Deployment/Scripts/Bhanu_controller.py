...
def quat_multiply(p, q):                                                                  #What are these p and q
...
class magdroneControlNode():
    ...
    def __init__(self):
        ...
        self.struct_x = 0.0         #for me its z instead of x=0 y,z,-x
        self.struct_y = 0.0
        self.struct_z = 2.00
        # Mission 1.
        ...
        self.q_CwD = [0.816, 0, 0.408, 0.408]
        self.q_CwD_i = [0.816, 0, -0.408, -0.408]
        self.t_CwD = [0.1, 0.025, -0.085, 0]
        ...
        self.state_pub = rp.Publisher(
            "/stag_state/pose", PoseStamped, queue_size=1)
        self.state_rate_pub = rp.Publisher(
            "/stag_state/rates", TwistStamped, queue_size=1)
    ...
    def stag_callback(self, data):
        if (len(data.stag_array) > 0):
           for i in range(len(data.stag_array)):
               if data.stag_array[i].id.data ==7: 


                  q_BwC = [data.stag_array[i].pose.orientation.w,   #.stag_array[i]
                           data.stag_array[i].pose.orientation.x,
                           data.stag_array[i].pose.orientation.y,
                           data.stag_array[i].pose.orientation.z]

                  t_BwC = [0,
                           data.stag_array[i].pose.position.x,
                           data.stag_array[i].pose.position.y,
                           data.stag_array[i].pose.position.z]
                  ...
                  filter_update_time = data.stag_array[i].header.stamp.secs + 1.0e-9 * data.stag_array[i].header.stamp.nsecs
    ...
    def update_error(self, X):
        # Get current pose wrt Aruco/## Stag
    ...
    def publish_state(self, X):
        ...
        stateMsg.header.frame_id = 'tag_7'    
        stateMsg.pose.position.x= X.item(0)           #check ".X' caps
        ...
        stateMsg.pose.orientation.x = q[1]            #check ".X' caps
        ...
        twistMsg.header.frame_id = 'tag_7'
    ...
    def send_commands(self):
        ...
        while not rp.is_shutdown():
            # Arm motors
            if self.arm > 0:
                rp.loginfo("Arming...")                     #loginfo is like print cmnd
            ...
            # Mission has started
            if self.on_mission:
                if (time.time() - self.lastOnline < 1.0):                              #what does this time calculate???
                    # Generate commands
                    ...
                    linear_z_cmd = self.clip_command(uZ + 0.5, 0.6, 0.4)                    #what does linear_zyxaz_cmd does
                    ...
                    linear_z_cmd = 0.5                  #maybe hover 0.5 =thrust
                    linear_y_cmd = 0.0
                    linear_x_cmd = 0.0
                    angular_z_cmd = 0.0
                ...
                cmd = TwistStamped()                                        #isnt twistMsg = TwistStamped()?? how again cmd

...
class magdroneControlNode():
    ...
    def send_commands(self):
        ...
        while not rp.is_shutdown():
            if self.cmds is not None:
                # Generate commands
                uZ = self.kd_z * self.z_error + self.kd_z * self.z_error_d
                uY = self.kd_y * self.y_error + self.kd_y * self.y_error_d
                uX = self.kd_x * self.x_error + self.kd_x * self.x_error_d
...
# Start Node
magdrone = magdroneControlNode()
>>>>>>> 715b535f73a2763f2a033e5f84feaf506afbbb9e
